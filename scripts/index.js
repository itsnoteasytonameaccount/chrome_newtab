"strict mode";
{
  ...
  const carousel = {
    ...
    get page() {
      return this._count
        ? Math.ceil(this.loadedBookmarks.length / this._count)
        : 0;
    },
    ...
    async updateCount() {
      ...
      const val = Math.floor((carouselElement.offsetWidth - 1) / 140) * 2;
      const padding = ((carouselElement.offsetWidth - 1) % 140) / 2;
      ...
      if (val !== this._count) {
        ...
        if (this.loadedBookmarks.length === 0) {
          const { children, id } = await loadBookmarks();
    ...
    blockTemplate: ({ title, url, id }) => {
      id = id ? `_id="${id}"` : "";
      const img = url
        ? `<img class="md-icon" src="chrome://favicon/size/32@1x/${url}">`
    ...
    initialPage(num) {
      let template = "";
      let i = num * this._count;
      const end =
        i + this._count >= this.loadedBookmarks.length
          ? this.loadedBookmarks.length
          : i + this._count;
      const selected = this.selectPage(num);
      ...
      if (addLi) {
        addLi.addEventListener("click", () => {
          ...
          if (!this.maskRendered) {
            ...
            bookmarks.getTree(
              maskRenderer.bind(null, [
                document.querySelector(".menu-function.exist"),
              ])
            );
          }
        });
      ...
      const actived = document.querySelector(
        `div.carousel div.switcher a.active`
      );
      actived && (actived.className = "");
      document.querySelector(
        `div.carousel div.switcher a:nth-child(${num + 1})`
      ).className = "active";
    },
    // 重设内容
    reset() {
    ...
    removeItem(id) {
      ...
      // 查找要被移除的元素
      for (
        ind = this._current * this._count;
        ind < loadedBookmarks.length;
        ind++
      ) {
      ...
      if (this.page < last) {
        ...
        document
          .querySelector(`div.carousel div.switcher a:nth-child(${last})`)
          .remove();
        // 当前页已销毁
      ...
      if (this._current < last - 1) {
        // 下一页的节点前移
        div.innerHTML = this.blockTemplate(
          loadedBookmarks[this._current * this._count + this._count - 1]
        );
    ...
    selectPage: (num) =>
      document.querySelector(`div.bookmarks ul:nth-child(${num + 1})`),
    has(id) {
      if (this.cached[id]) {
        return true;
      }

      const loadedBookmarks = this.loadedBookmarks;

      // 查找要被移除的元素
      for (let ind = 0; ind < loadedBookmarks.length; ind++) {
  ...
  const markTemplate = ({ id, title, url }) => {
    let img = url
      ? `<img class="md-icon" src="chrome://favicon/size/16@1x/${url}"></img>`
      : "";
  ...
  function maskRenderer(nodes, trees, /* 测试用参数 */ deep = 0) {
    ...
    // 尾递归
    if (newTrees.length) return maskRenderer(newNodes, newTrees, deep + 1);
  }
  ...
  function initialMask() {
    ...
    function clearAll(
      e,
      nodes = foreground.querySelectorAll("input[type='checkbox']:checked")
    ) {
    ...
    foreground
      .querySelector("button.sure")
      .addEventListener("click", function () {
        let nodes = foreground.querySelectorAll(
          ".item input[type='checkbox']:checked"
        );
        let type = foreground.querySelector("input[type='radio']:checked")
          .value;
        let inputs;
        if (type === "create") {
  ...
  contextMenus.onClicked.addListener(function (itemInfo) {
    if (itemInfo.menuItemId === "edit") {
      contextmenuEvent.call(document.querySelector("div.bookmarks"), {
        preventDefault() {},
      });
    }
  });
